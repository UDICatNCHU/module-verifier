...
- **「全年課」學分語義釐清(2026-04-23 發現)**: 24 個模組課程學分 ≥ 4 且學生普遍修兩學期,但備註無「選修兩學期」。兩類處理方式已就緒:
  - 若校方認為是「全年合計、兩學期都要修」→ 加備註「選修兩學期」,Bug #2 修復邏輯自動套用。**已處理**:應經系 3 模組的經濟學原理(commit `00cd85a`,18 位學生翻盤)。
  - 若校方認為是「全年課但只認某一學期」→ 用結構化欄位 `"認列學期": "上"` 或 `"下"`。**已處理**:物理系 / 機械系 3 模組的普通物理學(認列下學期)。
  - **仍待確認**:動物解剖生理學、材料科學導論、生物化學(4 模組)、史學方法、普通昆蟲學、成本與管理會計學等。
